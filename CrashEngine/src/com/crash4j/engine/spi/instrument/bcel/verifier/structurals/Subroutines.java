...
import com.crash4j.engine.spi.instrument.bcel.generic.IndexedInstruction;
			...
			for (int i=0; i<all.length; i++){
				if (colors.get(all[i]) == BLACK){
					...
					if (instructions_assigned.contains(all[i])){
						throw new StructuralCodeConstraintException("InstructionImpl '"+all[i]+"' is part of more than one subroutine (or of the top level and a subroutine).");
					}
					else{
