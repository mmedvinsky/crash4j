...
/**
 * Resource factory implementation for java.net.Socket!java.io.InputStream#read([B)I;
 */
public class java_net_Socket_java_io_InputStream_read__B_I_ extends any_Throughput_Integer implements ResourceBuilder
{
    @Override
			...
			Object instance, Object rv, ResourceClosure c) 
	{
        ...
        ResourceManagerSpi.raiseProtocolEvent(pr.beforeRead(spi, b, 0, b.length));
	}
			...
			long testStartTime, long actualDt, long dt) 
	{
		...
		{
	        byte[] b = (byte[])ArrayUtil.get(args,  0);
			ResourceManagerSpi.raiseProtocolEvent(pr.afterRead(spi, b, 0, b.length, (Integer)rv));
		}
	}
}
